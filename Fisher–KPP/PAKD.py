...
from HMM_clustering import FisherKPPHMMClustering
...
class PhaseTransitionSmoothnessLoss(nn.Module):
    ...
    def forward(self, student_outputs, gammas, time_indices=None):
        ...
        Args:
            student_outputs: Student predictions (batch_size, n_grid)
        ...
        if time_indices is not None:
            ...
            output_diff = student_outputs[1:] - student_outputs[:-1]  # (batch_size-1, n_grid)
        
        # Compute magnitude of change (L2 norm across grid points)
...
class PAKDDataset(Dataset):
    """Dataset with phase-aware weights from HMM for Fisher-KPP"""
...
def create_student_model(model_type, input_size, output_size, hidden_dim, num_blocks=1, dropout=0.0):
    """Create student model for Fisher-KPP"""
    if model_type == 'MLP':
...
def load_teacher_model(teacher_model_path, device):
    ...
    Returns:
        teacher_model, X_scaler, y_scaler, n_grid
    """
    ...
    if 'model_state_dict' in checkpoint:
        ...
        if num_blocks == 0:
            # MLP architecture
            num_blocks = sum(1 for k in state_dict.keys() if 'network.' in k and '.weight' in k) - 1
    
    # Reconstruct model
    if model_type == 'ResidualMLP':
        teacher_model = ResidualMLP(input_size=input_size, output_size=output_size, 
                                   hidden_dim=hidden_dim, num_blocks=num_blocks, dropout=0.0)
    else:
        teacher_model = MLP(input_size=input_size, output_size=output_size, 
                           hidden_sizes=[hidden_dim]*num_blocks, dropout=0.0)
    ...
    return teacher_model, X_scaler, y_scaler, n_grid
...
def create_time_features(time_array):
    ...
    Returns:
        log10(time + 1.0) with shape (n_samples, 1)
    """
    t = time_array
    t_log = np.log10(t + 1.0)
    return t_log.reshape(-1, 1)


def load_data_with_gammas(data_path, n_grid, run_hmm_if_missing=True, n_key_points=10):
    """
    ...
    Format: [time, u0_1-u0_n, u_1-u_n, gamma_0, gamma_1, ...]
    
    Returns:
        X: Features [time_log10, u0_1-u0_n] (n_grid+1 features)
        y: Targets [u_1-u_n] (n_grid outputs)
    ...
    # Expected columns: time (1) + u0 (n_grid) + u (n_grid) + gammas (n_phases)
    # Without gammas: 1 + 2*n_grid columns
    expected_cols_no_gammas = 1 + 2 * n_grid
    
    # Check if data contains gammas
    if data.shape[1] <= expected_cols_no_gammas:
        ...
        ICs = data[1:n_hmm_points+1, 1:n_grid+1]
        y = data[1:n_hmm_points+1, n_grid+1:2*n_grid+1]
        
        # Apply time feature engineering (log10)
        time_log10 = create_time_features(time_raw)
        X = np.concatenate([time_log10, ICs], axis=1)  # [log10(t), u0_1-u0_n]
        ...
        print(f"✓ Applied log10 time features: X={X.shape} [1 time + {n_grid} ICs]")
        ...
        data_with_gammas = np.column_stack([time_raw.reshape(-1, 1), ICs, y, gammas])
        ...
        time_raw = data[:, 0]
        ICs = data[:, 1:n_grid+1]
        y = data[:, n_grid+1:2*n_grid+1]
        gammas = data[:, 2*n_grid+1:]
        
        # Apply time feature engineering (log10)
        time_log10 = create_time_features(time_raw)
        X = np.concatenate([time_log10, ICs], axis=1)  # [log10(t), u0_1-u0_n]
        
        print(f"✓ Loaded data: X={X.shape} [1 time + {n_grid} ICs], y={y.shape}, gammas={gammas.shape}")
...
def train_epoch(student_model, dataloader, optimizer, loss_fn, device, hidden_layer='first'):
    ...
    for batch_data in dataloader:
        X_norm, y_teacher, teacher_hidden, phase_weights, gammas = batch_data
...
def evaluate_model(student_model, dataloader, device):
    ...
    with torch.no_grad():
        for X_norm, y_teacher, _, _, _ in dataloader:
    ...
    return r2, r2_grid, mae, rmse


def save_figures(all_losses, student_name, r2_grid, final_mae, final_rmse):
...
def visualize_hidden_alignment(student_model, dataloader, loss_fn, device, student_name, 
    ...
    with torch.no_grad():
        for X_norm, _, teacher_hidden, _, _ in dataloader:
            X_norm = X_norm.to(device)
            student_hidden = get_model_hidden_representation(student_model, X_norm, layer=hidden_layer)
...
# ---------------------------
def main():
    parser = argparse.ArgumentParser(description='Phase-Aware Knowledge Distillation for Fisher-KPP')
    ...
    parser.add_argument('--n_key_points', type=int, default=50,
                       help='Number of key grid points for HMM (if running on the fly)')
    ...
    # Load teacher model first to get n_grid
    teacher_model, X_scaler, y_scaler, n_grid = load_teacher_model(args.teacher_model, device)
    ...
    X, y, gammas, transition_matrix = load_data_with_gammas(
        ...
        n_key_points=args.n_key_points
    )
    ...
    print(f"  Type: {args.student_type}")
    print(f"  Input size: {input_size} (1 time + {n_grid} ICs)")
    print(f"  Output size: {output_size} ({n_grid} grid points)")
    print(f"  Hidden dim: {args.student_hidden_dim}")
    print(f"  Num blocks: {args.student_num_blocks}")
    ...
    # Evaluate
    r2, r2_grid, mae, rmse = evaluate_model(student_model, dataloader, device)
    ...
    # Save figures
    save_figures(all_losses, student_name, r2_grid, mae, rmse)
