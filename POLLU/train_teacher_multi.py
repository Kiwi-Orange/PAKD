...
class MultiConditionReactionDataset(Dataset):
    """Dataset for multi-condition chemical reaction trajectories"""
    def __init__(self, X, y, condition_labels=None):
        ...
        if condition_labels is None:
            initial_conditions = X[:, 1:21]  # y0_1 to y0_20
...
def create_time_features(time_array):
    ...
    # Single time transformation - log scale (good for exponential decay)
    t1 = np.log10(t + 1e-12)
...
# ============================================================================
def main():
    parser = argparse.ArgumentParser(description='Train teacher model on POLLU dataset')
    ...
    # Load data
    print("\nLoading POLLU dataset...")
    if args.file is None:
        data_files = [f for f in os.listdir('data/teacher') 
                     if f.startswith('teacher_pollu') and f.endswith('.npy')]
        if not data_files:
            raise FileNotFoundError("No POLLU dataset found. Run MAE_simulation.py first.")
    ...
    X = data[:, 0:21]   # time + 20 initial conditions
    y = data[:, 21:41]  # 20 species concentrations
    
    unique_conditions = np.unique(X[:, 1:21], axis=0)
    ...
    ])  # Total: 21 features (same as before)
    ...
    print(f"  ✓ Normalized all 21 features")
    ...
    # Normalize targets (y) - this replaces log transform
    y_scaler = StandardScaler()
    y_norm = y_scaler.fit_transform(y_processed)
    print(f"  ✓ Normalized targets (20 species)")
    ...
    # Create model (remove positive_output parameter)
    print(f"\nModel: {args.model}")
    ...
    model_name = f"{args.model}_pollu_{len(unique_conditions)}cond"
    ...
    print(f"  RMSE by species: {[f'{x:.4e}' for x in error_results['rmse_by_species'][:5]]}...")  # Show first 5
    ...
    torch.save({
        ...
        'train_losses': train_losses,
        'train_rmses': train_rmses,
        'input_size': 21,  # 1 time feature + 20 ICs
        'use_time_features': True,
        'time_feature_types': ['log10'],  # Only log10
