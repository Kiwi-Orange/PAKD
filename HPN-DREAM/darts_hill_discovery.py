...
# ============================================================
def discover_conditions(data_dir: str, source: str, condition_filter: str = None) -> List[Tuple[str, str]]:
    ...
    for fp in files:
        ...
        if condition_filter and condition_filter not in label:
            continue
        out.append((label, fp))
    return out
...
# ============================================================
# Shared-topology model (cross-condition joint training)
# ============================================================
class SharedTopology(nn.Module):
    """Shared across all conditions: gates G, Hill params K/n, sign, decay gamma, basal, Vmax."""
    def __init__(self, P: int, init_gate_bias: float = -2.0, gate_temperature: float = 1.5):
        super().__init__()
        self.P = P
        self.gates_mod = EdgeGates(P, init_bias=init_gate_bias, temperature=gate_temperature)
        self.logK = nn.Parameter(torch.zeros(P, P))
        ...
        masked_fill_diagonal_(self.logK.data, -10.0)
        masked_fill_diagonal_(self.logn.data, -10.0)
        self.sign_alpha = nn.Parameter(torch.zeros(P, P))
        masked_fill_diagonal_(self.sign_alpha.data, 0.0)
        self.loggamma = nn.Parameter(torch.zeros(P))
        self.basal = nn.Parameter(torch.zeros(P))
        self.logVmax = nn.Parameter(torch.zeros(P))
    ...
    def get_params(self) -> Dict[str, torch.Tensor]:
        ...
        Vmax = F.softplus(self.logVmax)
        gamma = F.softplus(self.loggamma)
        sign_gate = torch.sigmoid(self.sign_alpha)
        return dict(gates=g, K=K, n=n, Vmax=Vmax, gamma=gamma, sign_gate=sign_gate, basal=self.basal)

    def set_temperature(self, t: float):
        self.gates_mod.set_temperature(t)
...
class ConditionODE(nn.Module):
    """Wraps SharedTopology + condition-specific beta for ODE integration."""
    def __init__(self, shared: SharedTopology, use_additive: bool = True, use_multiplicative: bool = False):
        super().__init__()
        self.shared = shared
        self.P = shared.P
        self.use_add = use_additive
        self.use_mult = use_multiplicative
        self.beta = nn.Parameter(torch.zeros(self.P, self.P))
        masked_fill_diagonal_(self.beta.data, 0.0)
    ...
    def forward(self, t: torch.Tensor, X: torch.Tensor) -> torch.Tensor:
        ...
        H = self.shared._edge_h(Xb, K, n, sign_gate)
        decay = -gamma[None, :] * Xb

        add = 0.0
        if self.use_add:
            add = (g[None, :, :] * self.beta[None, :, :] * H).sum(dim=1) + self.shared.basal[None, :]
...
class TrainConfig:
    ...
    clip_grad: float = 2.0
    # Hybrid dual-engine
    phase1_epochs: int = 4000      # epochs for Phase 1 (derivative matching)
    phase2_epochs: int = 1000      # epochs for Phase 2 (ODE fine-tuning)
...
def train_one_condition(
    ...
    result, _ = _run_phase(model, p1_fit, cfg, phase1_epochs, 0, total_epochs, "P1 deriv")
    ...
    result, _ = _run_phase(
        model, p2_fit, cfg, phase2_epochs, phase1_epochs, total_epochs,
...
# ============================================================
# Cross-condition joint training (shared topology)
# ============================================================
def _run_phase_multi(
    shared: SharedTopology,
    cond_models: List[ConditionODE],
    loss_fn,
    cfg: TrainConfig,
    phase_epochs: int,
    epoch_offset: int,
    ...
    """Joint training phase across all conditions with shared topology."""
    t0 = _time.perf_counter()

    all_params = list(shared.parameters())
    for cm in cond_models:
        all_params.extend(cm.parameters())
    ...
    print(f"    ── {label} ({phase_epochs} ep, {len(cond_models)} conditions) ──")
    for ep in range(phase_epochs):
        global_ep = epoch_offset + ep
        temp = cosine_anneal(cfg.gate_temp_start, cfg.gate_temp_end, global_ep, total_epochs)
        shared.set_temperature(temp)

        opt.zero_grad(set_to_none=True)

        loss_fit = loss_fn()
        loss_sparse = cfg.lambda_sparse * shared.gates_mod.l1() / len(cond_models)
        loss_beta = sum(cfg.lambda_beta * cm.beta.abs().sum() for cm in cond_models) / len(cond_models)
        loss = loss_fit + loss_sparse + loss_beta
        ...
        if fit_val < best_fit - cfg.early_stop_min_delta:
            best_fit = fit_val
            best_state = {
                "shared": {k: v.detach().clone() for k, v in shared.state_dict().items()},
                "conds": [{k: v.detach().clone() for k, v in cm.state_dict().items()} for cm in cond_models],
            }
            no_improve = 0
        else:
            no_improve += 1
        ...
        if ep % 10 == 0 or ep == phase_epochs - 1:
            ...
            eta = elapsed / eps_done * (phase_epochs - eps_done)
            with torch.no_grad():
                g = shared.gates_mod.gates()
                n_edges = (g > cfg.gate_thr).float().sum().item()
                density = n_edges / (P * (P - 1))
            print(
                f"    [{label} ep {ep:04d}/{phase_epochs}] "
                f"loss={loss_val:.3e} fit={fit_val:.3e} "
                f"sparse={loss_sparse.item():.3e} temp={temp:.2f} "
    ...
    if best_state:
        ...
        for cm, state in zip(cond_models, best_state["conds"]):
            cm.load_state_dict(state)

    return {"loss": loss_val, "fit": fit_val}, _time.perf_counter() - t0


def train_multi_conditions(
    shared: SharedTopology,
    cond_data: List[Tuple[str, torch.Tensor, torch.Tensor]],
    cfg: TrainConfig,
    use_additive: bool = True,
    use_multiplicative: bool = False,
) -> Tuple[Dict[str, float], List[ConditionODE]]:
    """Train all conditions jointly with shared topology.

    cond_data: list of (label, t, y_true) tuples.
    Returns (result_dict, cond_models).
    """
    device = next(shared.parameters()).device
    P = shared.P
    cond_models = []
    cond_prepared = []

    for label, t, y_true in cond_data:
        t = t.to(device=device, dtype=torch.float32)
        y_true = y_true.to(device=device, dtype=torch.float32)
        x0 = y_true[0].detach()
        ...
        cond_prepared.append((label, t, y_true, x0, dYdt_target, tw_2d))
    ...
    # Phase 1: Joint derivative matching
    def p1_fit_v2():
        total = 0.0
        for i, (label, t, y_true, x0, dYdt_target, tw_2d) in enumerate(cond_prepared):
            total = total + (tw_2d * (cond_models[i](t, y_true) - dYdt_target) ** 2).mean()
        return total / len(cond_prepared)

    result, _ = _run_phase_multi(shared, cond_models, p1_fit_v2, cfg, phase1_epochs, 0, total_epochs, "P1 deriv")

    with torch.no_grad():
        g = shared.gates_mod.gates()
        n_edges = int((g > cfg.gate_thr).float().sum().item())
    print(f"    ── Phase 1 done: {n_edges} shared edges (gate>{cfg.gate_thr}) ──")

    if phase2_epochs <= 0:
        print(f"    ── Training complete: total={_time.perf_counter() - t0:.1f}s ──")
        return result, cond_models

    # Phase 2: Joint ODE fine-tuning
    def p2_fit():
        total = 0.0
        for i, (label, t, y_true, x0, _, tw_2d) in enumerate(cond_prepared):
            total = total + (tw_2d * (torchode_solve(
                cond_models[i], x0, t, method=cfg.ode_method, atol=cfg.ode_atol, rtol=cfg.ode_rtol,
            ) - y_true) ** 2).mean()
        return total / len(cond_prepared)

    result, _ = _run_phase_multi(
        shared, cond_models, p2_fit, cfg, phase2_epochs, phase1_epochs, total_epochs,
        f"P2 ODE ({cfg.ode_method})",
    )
...
def run_source(
    ...
    conds = discover_conditions(data_dir, source, condition_filter)
    if not conds:
...
# ============================================================
# Shared-topology pipeline
# ============================================================
@torch.no_grad()
def export_shared_model(shared: SharedTopology, cond_models: List[ConditionODE], cond_labels: List[str]) -> Dict:
    """Export shared topology + per-condition beta matrices."""
    p = shared.get_params()
    result = {
        "gates": p["gates"].cpu().numpy(),
        "K": p["K"].cpu().numpy(),
        "n": p["n"].cpu().numpy(),
        "sign_gate": p["sign_gate"].cpu().numpy(),
        "gamma": p["gamma"].cpu().numpy(),
        "basal": p["basal"].cpu().numpy(),
        "Vmax": p["Vmax"].cpu().numpy(),
    }
    for i, label in enumerate(cond_labels):
        result[f"beta_{label}"] = cond_models[i].beta.detach().cpu().numpy()
    return result


def run_source_shared(
    data_dir: str,
    output_dir: str,
    source: str,
    cfg: TrainConfig,
    ...
    """Run shared-topology pipeline: joint training across conditions."""
    conds = discover_conditions(data_dir, source, condition_filter)
    if not conds:
        raise FileNotFoundError(f"No files for source={source} in {data_dir}")

    out_src = os.path.join(output_dir, f"{source}_shared")
    os.makedirs(out_src, exist_ok=True)
    ...
    print(f"\n{'='*72}\nSOURCE={source} SHARED TOPOLOGY | conditions={len(conds)}\n{'='*72}")

    for ci, (label, fp) in enumerate(conds):
        print(f"\n-- Loading condition {ci+1}/{len(conds)}: {label}")
        t, X_raw, proteins = load_ts_csv(fp)
        if proteins_ref is None:
            proteins_ref = proteins
        else:
            assert proteins == proteins_ref, "Protein order mismatch"

        if normalize:
            X, p_mins, p_scales = normalize_proteins(X_raw)
            norm_info[label] = (p_mins, p_scales)
        else:
            X = X_raw

        tt = torch.tensor(t, dtype=torch.float32)
        YY = torch.tensor(X, dtype=torch.float32)
        cond_data.append((label, tt, YY))

    assert proteins_ref is not None
    P = len(proteins_ref)

    # Create shared topology + train jointly
    shared = SharedTopology(
        P=P,
        init_gate_bias=-2.0,
        gate_temperature=cfg.gate_temp_start,
    ...
    _, cond_models = train_multi_conditions(
        shared, cond_data, cfg,
        use_additive=use_additive,
        use_multiplicative=use_multiplicative,
    )

    # cond_models already have trained beta from training
    cond_labels = [label for label, _, _ in cond_data]
    mats = export_shared_model(shared, cond_models, cond_labels)

    # Save shared topology
    pd.DataFrame(mats["gates"], index=proteins_ref, columns=proteins_ref).to_csv(os.path.join(out_src, "GATES_shared.csv"))
    pd.DataFrame(mats["K"], index=proteins_ref, columns=proteins_ref).to_csv(os.path.join(out_src, "K_shared.csv"))
    pd.DataFrame(mats["n"], index=proteins_ref, columns=proteins_ref).to_csv(os.path.join(out_src, "n_shared.csv"))

    # Shared edges: use mean beta across conditions
    beta_keys = [k for k in mats if k.startswith("beta_")]
    mats["beta"] = np.mean([mats[k] for k in beta_keys], axis=0)
    shared_edges = export_edges_from_matrices(mats, proteins_ref, thr_gate=gate_thr)
    shared_edges.to_csv(os.path.join(out_src, "edges_shared.csv"), index=False)

    if len(shared_edges) > 0:
        G_net = build_graph_from_edges(shared_edges)
        plot_network(G_net, f"Shared Topology ({source})", os.path.join(out_src, "network_shared.png"))

    # Per-condition beta + edges
    for i, label in enumerate(cond_labels):
        cond_dir = os.path.join(out_src, "per_condition", label)
        os.makedirs(cond_dir, exist_ok=True)
        ...
        pd.DataFrame(cond_mats["beta"], index=proteins_ref, columns=proteins_ref).to_csv(os.path.join(cond_dir, "BETA.csv"))
        cond_edges = export_edges_from_matrices(cond_mats, proteins_ref, thr_gate=gate_thr)
        cond_edges.to_csv(os.path.join(cond_dir, "edges.csv"), index=False)

        eq_txt = equations_text(cond_mats, proteins_ref, topk=6, gate_thr=gate_thr)
        with open(os.path.join(cond_dir, "equations.txt"), "w", encoding="utf-8") as f:
            f.write(eq_txt)

    # Equations for shared topology (using mean beta already in mats["beta"])
    eq_txt = equations_text(mats, proteins_ref, topk=6, gate_thr=gate_thr)
    with open(os.path.join(out_src, "equations_shared.txt"), "w", encoding="utf-8") as f:
        f.write(eq_txt)

    n_shared = int((mats["gates"] > gate_thr).sum()) - P  # subtract diagonal
    print(f"\n✓ {source} shared topology done. {n_shared} edges (gate>{gate_thr})")
    print(f"  Saved under: {out_src}")
    return out_src
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="results/darts_hill_shared_topology",
    ...
    parser.add_argument("--lambda_sparse", type=float, default=1e-5,
                        help="L1 regularization on gate matrix G (1e-5 for single-condition mode, 0.0 for --shared_topology)")
    parser.add_argument("--lambda_beta", type=float, default=0.0,
                        help="L1 regularization on beta matrix (usually 0.0, since beta sparsity is controlled by G)")
    parser.add_argument("--temp_start", type=float, default=2.0)
    parser.add_argument("--temp_end", type=float, default=0.4)

    # export knobs
    parser.add_argument("--gate_thr", type=float, default=0.5)
    parser.add_argument("--consensus_freq", type=float, default=0.0,
                        help="Min fraction of conditions for consensus edge (old per-condition mode only, ignored with --shared_topology)")
    ...
    # hybrid dual-engine
    parser.add_argument("--phase1_epochs", type=int, default=3000,
                        help="Epochs for Phase 1 (derivative matching)")
    parser.add_argument("--phase2_epochs", type=int, default=500,
                        help="Epochs for Phase 2 (ODE fine-tuning)")

    # torchode integrator knobs (Phase 2)
    parser.add_argument("--ode_method", type=str, default="dopri5", choices=["tsit5", "dopri5"],
    ...
    parser.add_argument("--ode_atol", type=float, default=1e-3,
                        help="Absolute tolerance for adaptive step controller (1e-3 recommended for Phase 2 fine-tuning)")
    parser.add_argument("--ode_rtol", type=float, default=5e-2,
                        help="Relative tolerance for adaptive step controller (5e-2 gives 2-3x speedup over 1e-2)")
    ...
    # temporal weighting (0.0 = uniform, disabled by default)
    parser.add_argument("--time_weight_alpha", type=float, default=0.0,
                        help="Ramp exponent for temporal weighting (0=uniform/disabled, 0.5=gentle, 1.0=linear, >1=aggressive late-weighting)")
    parser.add_argument("--time_weight_min", type=float, default=0.3,
                        help="Floor weight for earliest time point (only used when alpha > 0)")
    ...
    # shared topology
    parser.add_argument("--shared_topology", action="store_true",
                        help="Joint training across conditions: share network topology (G, K, n, sign) while allowing condition-specific edge strengths (beta). "
                             "G directly represents the consensus regulatory structure present in all conditions, eliminating the need for post-hoc consensus voting. "
                             "Use when you have multiple experimental conditions and want to discover the common signaling pathway architecture.")

    # condition filter
    parser.add_argument("--condition", type=str, default=None,
                        help="Filter conditions by label substring (e.g. 'Serum|PD173074' to use only one condition). If not set, use all conditions.")
    ...
    run_fn = run_source_shared if args.shared_topology else run_source
    run_kwargs = dict(
    ...
    if not args.shared_topology:
        run_kwargs["consensus_freq"] = args.consensus_freq

    if args.source in ["teacher", "both"]:
        out_teacher = run_fn(source="teacher", **run_kwargs)
    if args.source in ["student", "both"]:
        out_student = run_fn(source="student", **run_kwargs)
