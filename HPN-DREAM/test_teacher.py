...
def set_publication_style():
    """Configure Nature Communications-style defaults for editable vector figures."""
    mpl.rcParams.update({
...
def save_publication_figure(fig: plt.Figure, output_dir: str, stem: str):
    """Save publication figures as editable vector files plus high-resolution PNG."""
...
# Colorblind-accessible palette for publication figures
COLORS = {
    'ground_truth': '#000000',
    'prediction': '#D55E00',
    'ci_band': '#D55E00',
...
# ============================================================================
# Publication-Quality Plotting (Nature Communications style)
# ============================================================================
def style_publication_axes(ax: plt.Axes, hide_top_right: bool = True):
    """Apply compact axis styling suitable for reduced manuscript panels."""
    if hide_top_right:
...
def format_log_time_axis(ax: plt.Axes, ticks: list = None):
    """Use fixed log-time ticks without scientific notation clutter."""
    if ticks is None:
        ticks = [5, 15, 30, 60, 120, 240]
    ticks = list(ticks)
    if min(ticks) <= 0:
        ax.set_xscale('symlog', linthresh=1)
        ax.set_xlim(min(ticks) - 1, max(ticks) * 1.18)
    else:
        ax.set_xscale('log')
        ax.set_xlim(min(ticks) * 0.82, max(ticks) * 1.18)
    ax.xaxis.set_major_locator(FixedLocator(ticks))
    ax.xaxis.set_major_formatter(FuncFormatter(lambda x, pos: f'{int(x)}' if x in ticks else ''))


def build_timecourse_summary(results: dict, aggregated_data: dict,
                             selected_stimuli: list,
                             time_points: list = None) -> dict:
    """
    Summarize no-inhibitor time courses by stimulus and time.

    This fixes raw-mode plotting by aggregating same-time replicates before drawing
    a line, so replicate order cannot create zig-zag trajectories.
    """
    if time_points is None:
        time_points = [5, 15, 30, 60, 120, 240]

    selected_set = set(selected_stimuli)
    time_set = set(float(t) for t in time_points)
    grouped = {}

    for idx, data in enumerate(aggregated_data.values()):
        stim = data['stimuli']
        inhib = data['inhibitors']
        time = float(data['time'])

        if stim not in selected_set or inhib != 'None' or time not in time_set:
            continue

        entry = grouped.setdefault(stim, {}).setdefault(time, {'gt': [], 'pred': []})
        y_all = np.asarray(data.get('y_all', [data['y_mean']]), dtype=float)
        if y_all.ndim == 1:
            y_all = y_all.reshape(1, -1)
        entry['gt'].append(y_all)
        entry['pred'].append(np.asarray(results['predictions'][idx], dtype=float))

    summary = {}
    for stim, by_time in grouped.items():
        summary[stim] = {}
        for time, values in by_time.items():
            gt_values = np.vstack(values['gt'])
            pred_values = np.vstack(values['pred'])

            gt_mean = np.mean(gt_values, axis=0)
            gt_sd = (
                np.std(gt_values, axis=0, ddof=1)
                if gt_values.shape[0] > 1
                else np.zeros(gt_values.shape[1])
            )

            summary[stim][time] = {
                'gt_mean': gt_mean,
                'gt_sd_low': gt_mean - gt_sd,
                'gt_sd_high': gt_mean + gt_sd,
                'prediction': np.mean(pred_values, axis=0),
                'n_replicates': gt_values.shape[0],
            }
...
def plot_predictions_vs_ground_truth_publication(results: dict, model_name: str, output_dir: str):
    """
    Publication-quality scatter plots: predictions vs ground truth.
    """
    predictions = results['predictions']
    gt_means = results['gt_means']
    ...
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(7.2, 1.05 * n_rows), squeeze=False)
    axes = axes.flatten()
    ...
    for i, protein in enumerate(protein_names):
        ...
        if np.any(~has_variability):
            ax.scatter(gt[~has_variability], pred[~has_variability], 
                      alpha=0.65, s=5, edgecolors='none',
        ...
        if np.any(has_variability):
            ax.errorbar(gt[has_variability], pred[has_variability], 
                       ...
                       fmt='o', markersize=1.8, alpha=0.75,
                       color=COLORS['prediction'], ecolor=COLORS['gray'],
                       elinewidth=0.45, capsize=1.2, label='Multi rep.')
        
        # Perfect prediction line
        min_val = min(gt.min(), pred.min())
        max_val = max(gt.max(), pred.max())
        margin = (max_val - min_val) * 0.06 if max_val > min_val else 0.1
        ax.plot([min_val - margin, max_val + margin], 
                [min_val - margin, max_val + margin], 
                color='black', linewidth=0.6, alpha=0.55, zorder=0)
    ...
    fig.tight_layout(w_pad=0.55, h_pad=0.7)
    save_publication_figure(fig, output_dir, 'pred_vs_gt_publication')
...
def plot_protein_performance_publication(results: dict, model_name: str, output_dir: str):
    ...
    ax1.set_yticklabels(display_names, fontsize=5)
    ax1.set_xlabel('$R^2$')
    ax1.set_title('$R^2$')
    ax1.axvline(x=0.5, color='black', linestyle='--', alpha=0.7, linewidth=0.7)
    ax1.axvline(x=0.2, color=COLORS['gray'], linestyle=':', alpha=0.75, linewidth=0.7)
    ax1.axvline(x=0, color=COLORS['gray'], linestyle='-', alpha=0.45, linewidth=0.6)
    ...
    ax3.barh(range(len(protein_names)), norm_rmse_plot, 
             color=colors_norm, edgecolor='none')
    ax3.set_yticks(range(len(protein_names)))
    ax3.set_xlabel('RMSE / bio. var.')
    ax3.set_title('Normalized RMSE')
    ax3.axvline(x=1.0, color='black', linestyle='--', alpha=0.7, linewidth=0.7)
    ax3.invert_yaxis()
    style_publication_axes(ax3)
    
    fig.tight_layout(w_pad=0.9)
    save_publication_figure(fig, output_dir, 'protein_performance_publication')
...
def plot_timecourse_with_ci_publication(results: dict, aggregated_data: dict,
    ...
    """
    Manuscript-ready time course plots for top proteins x fixed stimuli.
    """
    column_info = results['column_info']
    protein_names = column_info['protein_names']
    protein_metrics = results['protein_metrics']
    
    fixed_times = [5, 15, 30, 60, 120, 240]
    key_stimuli = ['EGF', 'Insulin', 'FGF1', 'HGF']
    timecourse = build_timecourse_summary(results, aggregated_data, key_stimuli, fixed_times)
    key_stimuli = [stim for stim in key_stimuli if stim in timecourse]
    ...
    for i, protein in enumerate(selected_proteins):
        ...
        for j, stim in enumerate(key_stimuli):
            ...
            times, gt_means, sd_low, sd_high, preds = [], [], [], [], []
            for time in fixed_times:
                point = timecourse.get(stim, {}).get(float(time))
                if point is None:
                    continue
                times.append(time)
                gt_means.append(point['gt_mean'][protein_idx])
                sd_low.append(point['gt_sd_low'][protein_idx])
                sd_high.append(point['gt_sd_high'][protein_idx])
                preds.append(point['prediction'][protein_idx])

            times = np.asarray(times, dtype=float)
            gt_means = np.asarray(gt_means, dtype=float)
            sd_low = np.asarray(sd_low, dtype=float)
            sd_high = np.asarray(sd_high, dtype=float)
            ...
            yerr = np.vstack([gt_means - sd_low, sd_high - gt_means])
            ax.errorbar(
                times, gt_means, yerr=yerr,
                fmt='o', markersize=3.2, linestyle='none',
                markeredgewidth=0.9, markeredgecolor=COLORS['ground_truth'],
                markerfacecolor='white', ecolor=COLORS['ground_truth'],
                elinewidth=0.65, capsize=1.8, capthick=0.65, zorder=5,
            )
            
            # Prediction (line)
            ax.plot(times, preds, color=COLORS['prediction'], linewidth=1.25, zorder=4)
            
            format_log_time_axis(ax, fixed_times)
            
            if i == 0:
                ax.set_title(stim, fontweight='bold', pad=3)
            if j == 0:
                display_name = PROTEIN_DISPLAY_NAMES.get(protein, protein)
                ax.set_ylabel(f'{display_name}\n$R^2$={r2:.2f}', fontweight='bold')
            if i == n_proteins - 1:
                ax.set_xlabel('Time (min)')
                ax.tick_params(axis='x', labelrotation=0)
            
            panel_values = np.concatenate([sd_low, sd_high, preds])
            ymin = np.nanmin(panel_values)
            ymax = np.nanmax(panel_values)
            span = ymax - ymin
            min_pad = max(0.03 * max(abs(ymin), abs(ymax), 1.0), 1e-3)
    ...
    legend_handles = [
        Line2D([0], [0], marker='o', linestyle='none', markersize=3.2,
               ...
               markeredgewidth=0.9, color='black', label='Experiment mean ± SD'),
        Line2D([0], [0], color=COLORS['prediction'], linewidth=1.25,
               label='Teacher prediction'),
    ]
    fig.legend(
        handles=legend_handles, loc='upper center', ncol=2, frameon=False,
        bbox_to_anchor=(0.5, 1.01), handlelength=1.8, columnspacing=1.2,
    )
    
    fig.tight_layout(rect=[0, 0, 1, 0.965], w_pad=0.9, h_pad=0.8)
    save_publication_figure(fig, output_dir, 'timecourse_with_ci')
    print(f"✓ Saved time course plot (4x4 NC style)")
...
def plot_error_heatmap_publication(results: dict, aggregated_data: dict,
    ...
    # RMSE heatmap
    ax1 = axes[0]
    sns.heatmap(error_matrix, annot=False, cmap='mako_r',
                xticklabels=display_names, yticklabels=unique_stimuli,
                ax=ax1, cbar_kws={'label': 'RMSE', 'shrink': 0.7})
    ax1.set_xlabel('Protein')
    ax1.set_ylabel('Stimulus')
    ax1.set_title('RMSE by stimulus and protein')
    plt.setp(ax1.get_xticklabels(), rotation=45, ha='right', fontsize=4.5)
    plt.setp(ax1.get_yticklabels(), fontsize=5.5)
    
    # R² heatmap
    ax2 = axes[1]
    sns.heatmap(r2_matrix, annot=False, cmap='viridis',
                      vmin=0, vmax=1,
                      xticklabels=display_names, yticklabels=unique_stimuli,
                      ax=ax2, cbar_kws={'label': '$R^2$', 'shrink': 0.7})
    ax2.set_xlabel('Protein')
    ax2.set_ylabel('Stimulus')
    ax2.set_title('$R^2$ by stimulus and protein')
    plt.setp(ax2.get_xticklabels(), rotation=45, ha='right', fontsize=4.5)
    plt.setp(ax2.get_yticklabels(), fontsize=5.5)
    
    fig.tight_layout(w_pad=1.6)
    save_publication_figure(fig, output_dir, 'error_heatmap_publication')
...
def plot_error_quantiles_over_time(results: dict, aggregated_data: dict,
    ...
    ax.semilogy(unique_times, rmse_q[1] + eps, linewidth=1.1,
    ...
    ax.semilogy(unique_times, mae_q[1] + eps, linewidth=1.0, linestyle='--',
               color=COLORS['blue'], label=r'MAE median')
    ax.fill_between(unique_times, mae_q[0] + eps, mae_q[2] + eps, 
                   alpha=0.18, color=COLORS['blue'], label=r'MAE 10–90%')
    
    ax.set_xlabel('Time (min)')
    ax.set_ylabel('Error')
    ax.set_title('Error over time')
    format_log_time_axis(ax, list(unique_times))
    ax.legend(frameon=False, loc='best')
    style_publication_axes(ax)
    
    fig.tight_layout()
    save_publication_figure(fig, output_dir, 'error_quantiles_time')
...
def plot_replicate_analysis(results: dict, aggregated_data: dict,
    ...
    for n_rep in unique_n_reps:
        ...
        ax1.scatter(np.full(mask.sum(), n_rep) + rng.normal(0, 0.08, mask.sum()),
                   condition_rmse[mask], alpha=0.55, s=7,
                   color=COLORS['prediction'], edgecolors='none')
    
    # Box plot overlay
    bp_data = [condition_rmse[n_replicates == n] for n in unique_n_reps]
    ax1.boxplot(
        bp_data, positions=unique_n_reps, widths=0.28, showfliers=False,
        medianprops={'color': 'black', 'linewidth': 0.8},
        boxprops={'color': 'black', 'linewidth': 0.65},
        whiskerprops={'color': 'black', 'linewidth': 0.65},
        capprops={'color': 'black', 'linewidth': 0.65},
    )
    
    ax1.set_xlabel('Replicates')
    ax1.set_ylabel('RMSE')
    ax1.set_title('Replicate count')
    style_publication_axes(ax1)
    ...
    if np.any(valid_mask):
        ax2.scatter(mean_bio_std[valid_mask], condition_rmse[valid_mask], 
                   alpha=0.55, s=7, color=COLORS['prediction'], edgecolors='none')
        
        # Add diagonal reference line (error = bio variability)
        max_val = max(mean_bio_std[valid_mask].max(), condition_rmse[valid_mask].max())
        ax2.plot([0, max_val], [0, max_val], '--', color='black', alpha=0.7, linewidth=0.8,
                label='Error = Bio. var.')
        ax2.legend(frameon=False)
    
    ax2.set_xlabel('Mean bio. std')
    ax2.set_ylabel('Model RMSE')
    ax2.set_title('Biological variability')
    style_publication_axes(ax2)
    ...
    if np.any(valid_mask):
        ...
        ax3.axvline(x=1.0, color='black', linestyle='--', linewidth=0.8,
                   label='Error = Bio. var.')
        ax3.axvline(x=np.median(normalized_error), color=COLORS['blue'], linestyle='-',
                   linewidth=0.8, label=f'Median = {np.median(normalized_error):.2f}')
        ax3.legend(frameon=False)
    else:
        ax3.text(0.5, 0.5, 'No replicate variability\n(raw mode: all n=1)', 
                transform=ax3.transAxes, ha='center', va='center')
    
    ax3.set_xlabel('RMSE / bio. std')
    ax3.set_ylabel('Count')
    ax3.set_title('Normalized error')
    style_publication_axes(ax3)
    
    fig.tight_layout(w_pad=0.9)
    save_publication_figure(fig, output_dir, 'replicate_analysis')
...
def plot_time_analysis_publication(time_results: dict, model_name: str, output_dir: str):
    ...
    # R² over time
    ax1 = axes[0]
    ax1.plot(times, r2_values, 'o-', linewidth=1.0, markersize=3.2,
            color=COLORS['good'], markeredgecolor='black', markeredgewidth=0.5)
    ax1.axhline(y=0.5, color='black', linestyle='--', alpha=0.7, linewidth=0.7)
    ax1.axhline(y=0.2, color=COLORS['gray'], linestyle=':', alpha=0.7, linewidth=0.7)
    ax1.set_xlabel('Time (min)')
    ax1.set_ylabel('$R^2$')
    ax1.set_title('$R^2$')
    format_log_time_axis(ax1, times)
    ax1.set_ylim(-0.1, 1.05)
    style_publication_axes(ax1)
    
    # RMSE over time
    ax2 = axes[1]
    ax2.plot(times, rmse_values, 's-', linewidth=1.0, markersize=3.2,
            color=COLORS['poor'], markeredgecolor='black', markeredgewidth=0.5)
    ax2.set_xlabel('Time (min)')
    ax2.set_ylabel('RMSE')
    ax2.set_title('RMSE')
    format_log_time_axis(ax2, times)
    style_publication_axes(ax2)
    
    # Sample count
    ax3 = axes[2]
    ax3.bar(range(len(times)), n_conditions, color=COLORS['blue'], edgecolor='none')
    ax3.set_xticks(range(len(times)))
    ax3.set_xticklabels([f'{t:.0f}' if t >= 1 else f'{t}' for t in times], rotation=45)
    ax3.set_xlabel('Time (min)')
    ax3.set_ylabel('Conditions')
    ax3.set_title('Sample count')
    style_publication_axes(ax3)
    
    fig.tight_layout(w_pad=0.9)
    save_publication_figure(fig, output_dir, 'time_analysis_publication')
